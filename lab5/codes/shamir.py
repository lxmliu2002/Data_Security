...
if __name__ == '__main__':
    ...
    s1=int(input("请输入秘密s1:"))
    s2=int(input("请输入秘密s2:"))
#秘密共享阶段：
    #为两个秘密s1,s2分别构造随机多项式f1(x)和f2(x)
    print('为秘密s1,s2构建随机多项式')
    ...
    for i in range(0,n):
        ...
        while temp in x:
            temp=random.randrange(0,p)
        x.append(temp)
    #计算得到shares=[xi,f1(xi),f2(xi)]，其中f1(xi)为秘密s1的秘密份额，f2(xi)为秘密s2的秘密份额
    shares=[]
    for i in range(0,n):
        shares.append([])
        shares[i].append(x[i])
...
#秘密重构阶段：
    ...
    #t个参与方分别将自己手中s1和s2的秘密份额相加，得到s1+s2的秘密份额
    shares_s1s2_x=[]
    shares_s1s2_y=[]
    print('s1+s2的秘密份额为：')
    for i in range(0,t):
